...
*Setup instructions for a gitian build of Searchcoin using a Debian VM or physical system.*
...
- [Building Searchcoin](#building-searchcoin)
...
Clone the git repositories for searchcoin and gitian and then checkout the searchcoin version that you want to build.
...
Follow the instructions in [doc/release-process.md](release-process.md) in the searchcoin repository
...
To build Searchcoin (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the searchcoin repository.
...
```bash
    ...
    From https://github.com/eastcoastcrypto/Searchcoin
...
For example:
```bash
URL=https://github.com/eastcoastcrypto/Searchcoin.git
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/searchcoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/searchcoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/searchcoin-build.assert
```
